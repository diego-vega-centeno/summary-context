...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "A real-time notification panel for the dashboard that pushes alerts without page refresh.",
          context:
            "Product team requested this after user feedback showed high refresh rates on the dashboard — users were manually refreshing to check for updates.",
        },
        key_decisions: [
          {
            decision:
              "Use native WebSocket API over a third-party library like Socket.io.",
            context:
              "@sofia_m argued for keeping the bundle size small. Discussed on Mar 6.",
            made_by: "@sofia_m",
          },
        ],
        blocking_points: [
          {
            point:
              "Team is split between WebSocket and Server-Sent Events (SSE) approach.",
            context:
              "@alex raised SSE as simpler and sufficient since notifications are server-to-client only. Discussion started Mar 9 and is still open.",
            waiting_on: "Team decision / @tech-lead",
            next_step:
              "Tech lead needs to make a final call to unblock the implementation direction.",
          },
        ],
        current_state:
          "PR is open, implementation is WebSocket-based but may need rework depending on team decision. 1 approval so far.",
        next_steps: [
...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "A Redis-backed rate limiting middleware applied to all public API routes to prevent abuse.",
          context:
            "A spike in API abuse was detected on Mar 1 — one client was making 10,000 requests/minute. This caused degraded performance for all other users.",
        },
        key_decisions: [
          {
            decision:
              "Use Redis for rate limit counters instead of in-memory storage.",
            context:
              "In-memory would not work across multiple API gateway instances. @dev_rami proposed Redis on Mar 8.",
            made_by: "@dev_rami",
          },
          {
            decision:
              "Apply a sliding window algorithm instead of fixed window.",
            context:
              "@priya pointed out that fixed window has edge cases at window boundaries. Agreed on Mar 9.",
            made_by: "@priya",
          },
        ],
        blocking_points: [
          {
            point:
              "Security team review required before any middleware touching public endpoints can be merged.",
            context:
              "Company policy established after a security incident in Q4 2023. @dev_rami submitted review request on Mar 10.",
            waiting_on: "@security-team",
            next_step:
              "Ping @security-team — SLA is 48 hours, now at 36 hours.",
          },
        ],
        current_state:
          "Implementation complete and tested. 3 approvals from engineering. Blocked on mandatory security review.",
        next_steps: [
...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "Migration of the payments service database connections from direct Postgres connections to PgBouncer connection pooling.",
          context:
            "Production incidents in January showed the service exhausting Postgres connection limits under peak load (Black Friday traffic simulation). Direct connections were not scalable.",
        },
        key_decisions: [
          {
            decision: "Use transaction mode pooling instead of session mode.",
            context:
              "Session mode would not reduce connection count enough. @john_arch benchmarked both on Feb 12.",
            made_by: "@john_arch",
          },
          {
            decision:
              "Deploy PgBouncer as a sidecar container rather than a separate service.",
            context:
              "Reduces network latency and simplifies deployment. Decided by @john_arch and @devops_lead on Feb 15.",
            made_by: "@devops_lead",
          },
        ],
        blocking_points: [
          {
            ...
            context:
              "Connections were not being returned to the pool after certain error paths. Found by @qa_team during stress test.",
            waiting_on: "Fix by @john_arch",
            next_step:
              "Was resolved — @john_arch patched the error handling on Feb 19.",
          },
        ],
        current_state:
          "Merged on Feb 20. Deployed to production Feb 21. Connection count reduced by 78% under peak load.",
        next_steps: [
...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "Full offline mode for the mobile app using SQLite local storage with background sync when connectivity is restored.",
          context:
            "Top requested feature from enterprise customers who use the app in low-connectivity environments (warehouses, field operations). Requested since Q3 2023.",
        },
        key_decisions: [
          {
            decision:
              "Use SQLite via expo-sqlite instead of AsyncStorage for structured offline data.",
            context:
              "AsyncStorage is key-value only and not suitable for relational data. Decision made on Jan 16.",
            made_by: "@lin_fe",
          },
          {
            decision:
              "Conflict resolution strategy: last-write-wins with server as source of truth.",
            context:
              "Simpler than operational transforms. Accepted trade-off for V1 of offline mode. Discussed Jan 22.",
            made_by: "@product_lead",
          },
        ],
        blocking_points: [],
        current_state:
          "Merged Feb 1. Released in v2.4 on Feb 5. No blocking issues during rollout.",
        next_steps: [
...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "Custom implementation of annual billing cycle logic inside the internal billing module.",
          context:
            "Sales team needed annual billing to close enterprise deals. Initially scoped as an internal feature.",
        },
        key_decisions: [
          {
            decision:
              "Abandon custom implementation in favor of Stripe Billing.",
            context:
              "After 2 weeks of development, @cto pointed out that Stripe Billing handles annual cycles natively and reduces maintenance burden. Decision made Feb 13.",
            made_by: "@cto",
          },
        ],
        blocking_points: [
          {
            point:
              "Proration logic for mid-cycle upgrades was significantly more complex than estimated.",
            context:
              "@petra_k raised this on Feb 10 after discovering edge cases with partial months. Estimated 2 more weeks of work.",
            waiting_on: "Was the trigger for the pivot decision.",
            next_step:
              "N/A — PR closed in favor of Stripe Billing integration.",
          },
        ],
        current_state:
          "PR closed Feb 14. A separate PR for Stripe Billing integration was opened by @john_arch (PR #312).",
        next_steps: [
...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "A Kafka consumer service to replace the current polling-based event ingestion in the data pipeline.",
          context:
            "The polling approach was causing up to 90 second delays in event processing. Real-time processing was required for a new analytics feature promised to customers in Q1.",
        },
        key_decisions: [
          {
            decision: "Use Kafka instead of RabbitMQ for the event bus.",
            context:
              "Kafka chosen for its replay capability and retention. Decision made by @data_lead on Jan 22 after evaluating both options.",
            made_by: "@data_lead",
          },
        ],
        blocking_points: [
          {
            point:
              "Kafka cluster has not been provisioned in the staging environment.",
            context:
              "@marco_b raised this on Feb 5. Infrastructure team said it was deprioritized due to a cloud cost freeze.",
            waiting_on: "@infra-team / cost freeze lifted",
            next_step:
              "Escalate to engineering manager — cost freeze was supposed to end Feb 28.",
          },
        ],
        current_state:
          "PR is complete and ready to test. Stale due to missing infrastructure. No activity since Feb 10.",
        next_steps: [
...

  {
    ...
    summary: {
      ...
      summary_json: {
        ...
        what_was_built: {
          summary:
            "Full migration of the search service from a self-hosted Elasticsearch cluster to Typesense Cloud to reduce ops overhead and cost.",
          context:
            "Elasticsearch cluster was costing $3,200/month and required dedicated DevOps attention. Typesense was evaluated in December and showed comparable performance at 1/5 the cost.",
        },
        key_decisions: [
          {
            decision:
              "Migrate in a shadow mode first — run both engines in parallel and compare results.",
            context:
              "@ana_search proposed this to de-risk the migration. Approved by @engineering_lead on Jan 8.",
            made_by: "@ana_search",
          },
          {
            decision:
              "Pause migration after shadow mode revealed ranking differences on fuzzy queries.",
            context:
              "On Jan 25, shadow mode showed Typesense returning different result ordering for 3-word fuzzy queries. Not a bug but a tuning issue.",
        ...
        blocking_points: [
          {
            point:
              "Typesense ranking for fuzzy multi-word queries does not match Elasticsearch behavior.",
            context:
              "@ana_search documented 12 edge cases on Jan 27. Typesense support was contacted but no response yet.",
            waiting_on:
              "Typesense support response + internal tuning decision.",
            next_step:
              "Decide whether to tune Typesense config to match ES behavior or accept the difference and update relevance tests.",
          },
        ],
        current_state:
          "Shadow mode is running. Migration is paused. PR has not been updated since Jan 28. No active discussion.",
        next_steps: [
